...
import 'package:gastos/data/repository/individual_expenses_repository.dart';
import 'package:gastos/data/shared_preferences_helper.dart';
import 'package:gastos/utils/date_formatter.dart';

class IndividualExpenseProvider with ChangeNotifier {
  //firebase
  IndividualExpenseProvider();
  Future<void> initialLoad(String firebaseUID) async {
    _dateType = dateType;
  ...
  //variables
  final repository = IndividualExpensesRepository();
  ...
  Future<void> get(String firebaseUID,[int offset = 0]) async {
    ...
    try {
      // _expenses.clear();
      _expenses.addAll(await repository.readAll(dateType,firebaseUID, offset));
    } catch (err) {
      rethrow;
    } finally {
      loading = false;
  ...
  Future<void> getByDateType(DateType type,String firebaseUID) async {
    offset = 0;
    dateType = type;
  ...
  Future<void> getByScroll(String firebaseUID) async {
    if (blockFunction | blockInfiniteScroll) return;
    blockFunction = true;
    offset += 100;
    int count = await repository.countRows(firebaseUID);
    if (offset - 100 >= count) {
    ...
    final type = preferences.getDateType();
    final newList = await repository.getByScroll(firebaseUID,offset);
