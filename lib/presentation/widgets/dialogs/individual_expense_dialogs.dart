...
class IndividualExpenseDialog extends StatelessWidget
    with MaterialStatePropertyMixin {
  const IndividualExpenseDialog(
...
class _ExpenseHandlerContentState extends State<ExpenseHandlerContent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      child: Column(
        ...
        children: [
          const Text('Añadir gasto individual'),
  ...
  Future<void> createExpense(
    ...
    try {
      await state.add(expense: expense, individual: true);
    } catch (err) {
      if (kDebugMode) {
        print(err);
      }
  ...
  Future<void> updateExpense(
    ...
    try {
      ...
      await state.update(expense: newExpense, individual: true);
      Navigator.of(context).pop();
    } catch (err) {
      if (kDebugMode) {
        print(err);
